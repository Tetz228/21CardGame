...
namespace Game21
{
    ...
    class Program
    {
        ...
        static bool MenuScreen(bool enableMusic)
        {
            ...
            menu.DisplayingMenuItems(menuItems, MenuItemNumber = 1);
            ...
            while (item != true)
            {
                ...
                else if (key.Key == ConsoleKey.Enter)
                {
                    ...
                    {
                        About(menuItems, MenuItemNumber);
                    }
                    else Environment.Exit(0);
                }
                Console.ResetColor();

                menu.DisplayingMenuItems(menuItems, MenuItemNumber);
            }
            if (item == false)
            {
                return false;
        ...
        #region Окно "About"
        static void About(Array menuItems, int MenuItemNumber)
        {
        ...
        #region Постоянная смена цвет у "Author: Bereberdin Mikhail" (меняет цвет 6 раз)
        static void rainbow()
        {
            for (int i = 0; i < 6; i++)
            {
        ...
        #region Ввод имени пользователя
